--[[
    Nothing Hub UI Library (UI-only, extracted — no farm/skill/combat logic)
    Host this file somewhere raw text is served (GitHub raw, Pastebin raw, etc.)
    then load it like this from any script:

        local create = loadstring(game:HttpGet("https://your-raw-link-here"))()
        local Window = create:Win("My Hub")
        local Tab1 = Window:Taps("Main")
        local Page1 = Tab1:newpage()
        Page1:Toggle("Example Toggle", false, function(state) end)
        Page1:Button("Example Button", function() end)
        create:Notifile("Title", "Some message", 5)

    Window height has been reduced (shorter default size) for mobile screens.
]]
...
function create:Win(text)
    ...
    tween:Create(MainSceen,TweenInfo.new(0.4,Enum.EasingStyle.Back),{Size = UDim2.new(0, 550, 0, 380)}):Play()
        ...
        pcall(function()
            if input.KeyCode == Enum.KeyCode.RightControl then
                if library.toggledui == false then
                    ...
                    tween:Create(MainSceen,TweenInfo.new(0.4,Enum.EasingStyle.Back),{Size = UDim2.new(0, 534, 0, 380)}):Play()
    ...
    MainSceen2:TweenSizeAndPosition(UDim2.new(0, 550-20, 0, 380-50), UDim2.new(0.5, 0, 0.53, 0), "Out", "Back", 0.5, true)
    ...
    Container_Page.Size = UDim2.new(0, 518, 0, 220)
...
--Time.AnchorPoint = Vector2.new(0.5, 0.5)
    ...
    function top:Taps(text)
        ...
        MainPage.Size = UDim2.new(0, 518, 0, 280)
        ...
        ScrollingFrame_Pagefrist.Size = UDim2.new(0, 518, 0, 280)
        ...
        UIGridLayout_Pagefrist.CellSize = UDim2.new(0, 240, 0, 275)
        ...
        function page:newpage()
            ...
            Pageframe.Size = UDim2.new(0, 240, 0, 284)
            ...
            ScrollingFrame_Pageframe.Size = UDim2.new(0, 240, 0, 284)
    ...
    TextButton_Toggle2.MouseButton1Click:Connect(function()
        ...
        function items:Slider(text,floor,min,max,de,lol,callback)
            ...
            SliderFrame.Size = UDim2.new(0, 213, 0, 75)
            ...
            -- CustomValue is a real TextBox: the user can tap it and type a value directly (mobile-friendly)
            ...
            CustomValue.Text = tostring(de or min or 0)
            ...
            -- MainValue: visual fill bar, purely display — updated by +/-/textbox, not draggable
            local MainValue = Instance.new("Frame")

            MainValue.Name = "MainValue"
            MainValue.Parent = PartValue
            MainValue.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            MainValue.Size = UDim2.new(math.clamp(((de or min or 0) - min) / (max - min), 0, 1), 0, 0, 5)
            ...
            -- ConneValue: draggable handle — works with both mouse (desktop) and touch (mobile)
            ...
            ConneValue.Position = UDim2.new(math.clamp(((de or min or 0) - min) / (max - min), 0, 1), 0, 0.5, 0)
            ConneValue.ClipsDescendants = false
            ConneValue.Active = true
            ...
            -- Refreshes the fill bar + handle position from the current CustomValue.Text, then fires callback
            local function ApplyValue()
                local n = tonumber(CustomValue.Text) or min
                if n > max then n = max end
                if n < min then n = min end
                CustomValue.Text = floor and string.format("%.1f", n) or tostring(math.floor(n))
                local scale = math.clamp((n - min) / (max - min), 0, 1)
            ...
            -- Sets the value directly from a drag/touch input position along ValueFrame
            local function SetValueFromInput(input)
                local scale = math.clamp((input.Position.X - ValueFrame.AbsolutePosition.X) / ValueFrame.AbsoluteSize.X, 0, 1)
                local n = (scale * (max - min)) + min
                if n > max then n = max end
                if n < min then n = min end
                CustomValue.Text = floor and string.format("%.1f", n) or tostring(math.floor(n))
                local snappedScale = math.clamp((n - min) / (max - min), 0, 1)
                MainValue:TweenSize(UDim2.new(snappedScale, 0, 0, 5), "Out", "Sine", 0.1, true)
                ConneValue:TweenPosition(UDim2.new(snappedScale, 0, 0.5, 0), "Out", "Sine", 0.1, true)
                callback(n)
            end
            ...
            CustomValue.FocusLost:Connect(function()
                if CustomValue.Text == "" then
                    CustomValue.Text = tostring(de or min)
                end
                ApplyValue()
            end)
...
ScolDown.CanvasSize = UDim2.new(0,0,0,UIListLayoutlist.AbsoluteContentSize.Y + 10  )
        ...
        function items:MultiDrop(text,option,callback)

            local DropFrame = Instance.new("Frame")

            DropFrame.Name = "MultiDropFrame"
            DropFrame.Parent = ScrollingFrame_Pageframe
            DropFrame.BackgroundColor3 =  Color3.fromRGB(23, 23, 23)
            DropFrame.BorderSizePixel = 0
            DropFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
            DropFrame.AnchorPoint = Vector2.new(0.5, 0.5)
            DropFrame.Size = UDim2.new(0, 213, 0, 30)
            DropFrame.BackgroundTransparency  = 0
            DropFrame.ClipsDescendants = true
            ...
            TextLabel_TapDrop.BackgroundTransparency = 1
            TextLabel_TapDrop.TextXAlignment = Enum.TextXAlignment.Left
            TextLabel_TapDrop.TextTruncate = Enum.TextTruncate.AtEnd
            ...
            local function RefreshLabel()
                ...
                if #names == 0 then
                    ...
                    TextLabel_TapDrop.Text = tostring(text)..": "..table.concat(names, ", ")
                end
            end
            ...
            local function BuildRow(v)
                local FrameSize = 75
                local cout = 0
                for i in pairs(ItemRows) do cout = cout + 1 end
                cout = cout + 1
                if cout == 1 then
                    FrameSize = 75
                elseif cout == 2 then
                ...
                TextLabel_TapDro2p.Parent = ListFrame
                TextLabel_TapDro2p.Name =  tostring(v).."MultiDropdown"
                TextLabel_TapDro2p.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                TextLabel_TapDro2p.Position = UDim2.new(0.5, 0, 0.5, 0)
                TextLabel_TapDro2p.Size =  UDim2.new(0, 180, 0, 30)
                TextLabel_TapDro2p.Font = Enum.Font.SourceSansSemibold
                TextLabel_TapDro2p.Text = tostring(v)
                TextLabel_TapDro2p.TextColor3 = Selected[v] and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(155, 155, 155)
                TextLabel_TapDro2p.TextSize = 14.000
                TextLabel_TapDro2p.AnchorPoint = Vector2.new(0.5, 0.5)
                TextLabel_TapDro2p.BackgroundTransparency = 1
                TextLabel_TapDro2p.TextXAlignment = Enum.TextXAlignment.Center
                ...
                Checkmark.Parent = ListFrame
                Checkmark.Name = "Checkmark"
                Checkmark.BackgroundTransparency = 1
                Checkmark.AnchorPoint = Vector2.new(0.5, 0.5)
                Checkmark.Position = UDim2.new(0.92, 0, 0.5, 0)
                Checkmark.Size = UDim2.new(0, 14, 0, 14)
                Checkmark.Image = "http://www.roblox.com/asset/?id=6031094678"
                Checkmark.ImageColor3 = Color3.fromRGB(120, 220, 140)
                Checkmark.Visible = Selected[v] == true
                ...
                ButtonDrop2.MouseButton1Click:Connect(function()
                    ...
                    TweenService:Create(
                        TextLabel_TapDro2p,
                        TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                        {TextColor3 = Selected[v] and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(155, 155, 155)}
                    ):Play()
                    RefreshLabel()
                    callback(Selected)
                end)

                ItemRows[v] = ListFrame
                ScolDown.CanvasSize = UDim2.new(0,0,0,UIListLayoutlist.AbsoluteContentSize.Y + 10  )
                return FrameSize
            end
            ...
            ButtonDrop.MouseButton1Click:Connect(function()
                if dog == false then
                    TweenService:Create(
                        DropFrame,
                        TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                        {Size = UDim2.new(0, 213, 0, FrameSize)}
                    ):Play()
                    TweenService:Create(
                        DropArbt_listimage,
                        TweenInfo.new(0.3, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
            ...
            -- Clear() resets every selected item back to unselected and refreshes the label
            function multidropfunc:Clear()
                for v in pairs(Selected) do
                    Selected[v] = nil
                end
                for v, ListFrame in pairs(ItemRows) do
                    local lbl = ListFrame:FindFirstChild(tostring(v).."MultiDropdown")
                    local mark = ListFrame:FindFirstChild("Checkmark")
                    if lbl then lbl.TextColor3 = Color3.fromRGB(155, 155, 155) end
            ...
            -- GetSelected() returns the current selection table {["Item"] = true, ...}
            function multidropfunc:GetSelected()
                return Selected
            end
